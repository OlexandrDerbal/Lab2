...
int main() {
    ...
    cout << "1 — Формула Векторного добутку\n";
    cout << "2 — Формула Герона\n";
    cout << "Ваш вибiр: ";
    cin >> method;

    if (method == 1) {
        cout << "Площа трикутника (Векторний добуток): " << area(t) << endl;
    }
    else if (method == 2) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (isOnEdge(t, p)) {
            cout << "Точка знаходиться на межi трикутника." << endl;
        }
        else if (isInside(t, p)) {
            cout << "Точка знаходиться всерединi трикутника\n";
        }
        else {
